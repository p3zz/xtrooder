...
// used for X/Y axis
pub fn dps_from_radius(r: Vector, steps_per_revolution: u64) -> Vector {
    let p = 2.0 * r.to_mm() * PI;
    Vector::from_mm(p / (steps_per_revolution as f64))
}

// get distance per step from bar's pitch
// used for Z axis
pub fn dps_from_pitch(pitch: Vector, steps_per_revolution: u64) -> Vector {
    Vector::from_mm(pitch.to_mm() / (steps_per_revolution as f64))
}
...
pub fn compute_step_duration(spr: u64, dps: Vector, speed: Vector) -> Duration {
