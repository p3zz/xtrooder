...
>(
    ...
    let e_speed = Speed::from_mm_per_second(e_distance / a_time);
...
>(
    ...
    let ab_speed_x = direction.unwrap().get_x().to_mm() * speed.get_x().to_mm_per_second();
    let ab_speed_y = direction.unwrap().get_y().to_mm() * speed.get_y().to_mm_per_second();
