...
async fn read_file_512_blocks() {
    ...
    let test_file = volume_mgr
        .open_file_in_dir(test_dir, "TEST.DAT", Mode::ReadOnly).await
    ...
    while !volume_mgr.file_eof(test_file).expect("check eof") {
        ...
        let len = volume_mgr.read(test_file, &mut buffer).await.expect("read data");
        if len != buffer.len() {
            if partial {
...
async fn read_file_all() {
    ...
    let len = volume_mgr
        .read(test_file, &mut contents).await
        .expect("read data");
    if len != 3500 {
