...
use fs::{blockdevice::{BlockCount, BlockDevice, BlockIdx, BlockTrait}, filesystem::timestamp::{TimeSource, Timestamp}, DeviceError, BLOCK_LEN};
...
///
/// The slice should be a multiple of `embedded_sdmmc::BLOCK_LEN as usize` bytes in
    ...
    T: AsMut<[u8]> + AsRef<[u8]>,
{
    ...
    async fn read(
        ...
        for block in blocks.iter_mut() {
            let start_offset = block_idx.0 as usize * BLOCK_LEN as usize;
            let end_offset = start_offset + BLOCK_LEN as usize;
            if end_offset > contents.len() {
                return Err(DeviceError::DeviceError(Error::OutOfBounds(block_idx)));
            }
            block
                .content_mut()
    ...
    async fn write(
        ...
        for block in blocks.iter() {
            ...
            contents[start_offset..end_offset].copy_from_slice(block.content());
            block_idx.0 += 1;
        }
        Ok(())
    }
    ...
    async fn num_blocks(&self) -> Result<BlockCount, DeviceError<Self::E>> {
        let borrow = self.contents.borrow();
        let contents: &[u8] = borrow.as_ref();
        let len_blocks = contents.len() as usize / BLOCK_LEN as usize;
