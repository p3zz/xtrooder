...
use fs::{filesystem::{directory::DirEntry, filename::ShortFileName, files::Mode}, volume_mgr::{VolumeIdx, VolumeManager}, DeviceError};
...
async fn open_dir_twice() {
    ...
    assert!(matches!(
        ...
        Err(DeviceError::OpenedFileAsDir)
    ));
...
async fn open_too_many_dirs() {
    ...
    assert!(matches!(
        ...
        Err(DeviceError::TooManyOpenDirs)
    ));
}
...
async fn find_dir_entry() {
    ...
    assert!(matches!(
        volume_mgr.find_directory_entry(root_dir, "README.TXS").await,
...
async fn delete_file() {
    ...
    assert!(matches!(
        volume_mgr.delete_file_in_dir(root_dir, "README.TXT").await,
        Err(DeviceError::FileAlreadyOpen)
    ));

    assert!(matches!(
        volume_mgr.delete_file_in_dir(root_dir, "README2.TXT").await,
        Err(DeviceError::NotFound)
    ));
    ...
    volume_mgr
        .delete_file_in_dir(root_dir, "README.TXT").await
        .unwrap();
...
async fn make_directory() {
    ...
    volume_mgr
        .make_dir_in_dir(root_dir, &test_dir_name).await
        .unwrap();
    ...
    volume_mgr
        .write(new_file, b"Hello").await
        .expect("write to new file");
    volume_mgr.close_file(new_file).await.expect("close new file");
    ...
    let new_dir = volume_mgr
        .open_dir(root_dir, &test_dir_name).await
        .expect("find new dir");
    let new_file = volume_mgr
        .open_file_in_dir(new_dir, &test_file_name, Mode::ReadOnly).await
