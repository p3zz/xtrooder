...
mod tests {
    ...
    use crate::{planner::motion, stepper::a4988::{Stepper, StepperAttachment, StepperOptions, SteppingMode}};
    ...
    #[test]
    fn test_stepper_move_counterclockwise(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let steps = 20;
        s.0.reset();
    ...
    #[test]
    fn test_stepper_move_microstepping_clockwise(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let steps = 20;
        s.0.reset();
    ...
    #[test]
    fn test_stepper_move_microstepping_counterclockwise(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let steps = 20;
        s.0.reset();
        s.0.set_stepping_mode(SteppingMode::HalfStep);
        s.0.set_direction(RotationDirection::CounterClockwise);
    ...
    #[test]
    fn test_stepper_move_clockwise_positive_direction_clockwise(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let steps = 20;
        s.0.reset();
    ...
    #[test]
    fn test_stepper_move_clockwise_positive_direction_counterclockwise(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let steps = 20;
        s.0.reset();
        s.0.set_stepping_mode(SteppingMode::FullStep);
        s.0.set_direction(RotationDirection::Clockwise);
    ...
    #[test]
    fn test_stepper_move_counterclockwise_positive_direction_clockwise(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let steps = 20;
        s.0.reset();
        s.0.set_stepping_mode(SteppingMode::FullStep);
        s.0.set_direction(RotationDirection::CounterClockwise);
    ...
    #[test]
    fn test_stepper_move_counterclockwise_positive_direction_counterclockwise(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let steps = 20;
        s.0.reset();
        s.0.set_stepping_mode(SteppingMode::FullStep);
        s.0.set_direction(RotationDirection::CounterClockwise);
    ...
    #[test]
    fn test_stepper_move_for_distance_no_attachment(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
    ...
    #[test]
    fn test_stepper_move_for_distance(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
    ...
    #[test]
    fn test_stepper_move_for_distance_space_wasted(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
    ...
    #[test]
    fn test_stepper_move_for_distance_lower_distance_per_step(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let distance = Distance::from_mm(10.5);
        s.0.reset();
    ...
    #[test]
    fn test_stepper_move_for_distance_negative(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let distance = Distance::from_mm(-10.5);
        s.0.reset();
        s.0.set_attachment(StepperAttachment { distance_per_step: Distance::from_mm(0.5) });
        let res = s.0.move_for_distance(distance);
    ...
    #[test]
    fn test_stepper_home_no_attachment(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        s.0.reset();
        s.0.set_stepping_mode(SteppingMode::FullStep);
    ...
    #[test]
    fn test_stepper_set_speed_positive(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        s.0.reset();
        s.0.set_stepping_mode(SteppingMode::FullStep);
    ...
    #[test]
    fn test_stepper_set_speed_negative(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        s.0.reset();
        s.0.set_stepping_mode(SteppingMode::FullStep);
    ...
    #[test]
    fn test_linear_move_to_negative_speed(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let destination = Distance::from_mm(-10.0);
        let speed = Speed::from_mm_per_second(-10.0);
        s.0.reset();
        s.0.set_attachment(StepperAttachment { distance_per_step: Distance::from_mm(1.0) });
    ...
    #[test]
    fn test_linear_move_to_2d_different_stepping_mode(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let destination = Vector2D::new(Distance::from_mm(-5.0), Distance::from_mm(5.0));
        let speed = Speed::from_mm_per_second(10.0);
        s.0.reset();
        s.1.reset();
        ...
        assert_eq!(s.1.get_direction(), RotationDirection::Clockwise);
        assert_eq!(s.0.get_speed_from_attachment().unwrap().to_mm_per_second(), 7.07734118446382);
        assert_eq!(s.1.get_speed_from_attachment().unwrap().to_mm_per_second(), 7.074337134610486);
    }

    #[test]
    fn test_linear_move_to_3d(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let destination = Vector3D::new(Distance::from_mm(-5.0), Distance::from_mm(5.0), Distance::from_mm(5.0));
        let speed = Speed::from_mm_per_second(10.0);
        s.0.reset();
        s.1.reset();
        s.2.reset();
    ...
    #[test]
    fn test_linear_move_to_3d_lower_distance_per_step(s: &mut (Stepper<'static>, Stepper<'static>, Stepper<'static>)) {
        let destination = Vector3D::new(Distance::from_mm(-5.0), Distance::from_mm(-2.0), Distance::from_mm(5.0));
        let speed = Speed::from_mm_per_second(10.0);
        s.0.reset();
        s.1.reset();
        s.2.reset();
        s.0.set_attachment(StepperAttachment { distance_per_step: Distance::from_mm(0.5) });
        s.1.set_attachment(StepperAttachment { distance_per_step: Distance::from_mm(0.5) });
        s.2.set_attachment(StepperAttachment { distance_per_step: Distance::from_mm(0.5) });
        s.0.set_stepping_mode(SteppingMode::FullStep);
        s.1.set_stepping_mode(SteppingMode::FullStep);
        s.2.set_stepping_mode(SteppingMode::FullStep);
        let res = motion::linear_move_to_3d(&mut s.0, &mut s.1, &mut s.2, destination, speed);
        ...
        assert_eq!(s.1.get_direction(), RotationDirection::CounterClockwise);
        assert_eq!(s.2.get_direction(), RotationDirection::Clockwise);
        assert_eq!(s.0.get_speed_from_attachment().unwrap().to_mm_per_second(), 9.277470590418229);
        assert_eq!(s.1.get_speed_from_attachment().unwrap().to_mm_per_second(), 3.725338260714073);
        assert_eq!(s.2.get_speed_from_attachment().unwrap().to_mm_per_second(), 7.07734118446382);
    }
